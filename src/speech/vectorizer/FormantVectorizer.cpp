...
std::valarray<double> speech::vectorizer::FormantVectorizer<FrameType>::vectorize(FrequencySample<FrameType> &sample) {
    // F0 boundaries
    int pos100Hz = sample.getFrequencyIndex(100.0);
    ...
    double *f0FormantPos = std::max_element(amplitudePtr + pos100Hz, amplitudePtr + pos300Hz);
    ...
    result[0] = (sample.getIndexFrequency(f0FormantPos - amplitudePtr));
